...
class MacOSNotifier(Notifier):
    ...
    def notifier(
        ...
        if shutil.which("osascript") is not None:
            safe_message = _escape_applescript(_normalize_osascript_text(message))
            safe_title = _escape_applescript(_normalize_osascript_text(title))
            script = (
...
def _normalize_osascript_text(value: str) -> str:
    # `display notification` doesn't accept raw newlines in a quoted string.
    # Flatten control whitespace to keep fallback delivery robust.
    return (
        value.replace("\r\n", "\n")
        .replace("\r", "\n")
        .replace("\n", " ")
